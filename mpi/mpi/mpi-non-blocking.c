...
int main(int argc, char *argv[]) {
	int id, nprocs, N = 512, n, process_id; // area divided into n trapezoids
	...
	if (id != 0) { // slave processes
		MPI_Isend(&my_area, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &r);
		MPI_Wait(&r, MPI_STATUS_IGNORE);
	} 
	else { // cosider process 0 as root process
		double areas[nprocs+1];
		areas[id] = my_area;
		memset(areas, 0, sizeof(areas));
		for (process_id = 1; process_id < nprocs; process_id++) {
			MPI_Irecv(&areas[process_id], 1, MPI_DOUBLE, process_id, 0, MPI_COMM_WORLD, &r);
			MPI_Wait(&r, MPI_STATUS_IGNORE);
		}
		total_area = 0;
